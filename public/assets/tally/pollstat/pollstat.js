...
var pollstat = function () {

    var tableID = 'table_pollstat';
    var page_slug = 'pollstat';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"pollstat":"lg"};
    ...
    var get_url = function (param) {
        ...
        return panel_url('pollstat' + param);
    ...
    //Module Lang
    var pollstatLang = function (param, paramsub) {
        return getLang('pollstat', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_pollstat').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="pollstatselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="pollstatselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
                ...
                {   
                ...
                },
                    {
                        ...
                        data: function (row) {
                            ...
                            //pollstat form button
                            
                            $link += `<a
                               href="` + get_url('showForm/pollstat/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_pollstat"
                               data-modalurl="` + get_url('showForm/pollstat/' + id) + `"
                               data-modaldata='{"id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + pollstatLang('_form_pollstat') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#pollstatselectAll');
        ...
        //Form
        $('#form_pollstat').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_pollstat').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return pollstatLang(param, paramsub);
        },
