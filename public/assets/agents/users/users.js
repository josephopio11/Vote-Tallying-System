...
var users = function () {
    ...
    var tableID = 'table_users';
    var page_slug = 'users';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"users":"lg"};
    ...
    //Module Lang
    var usersLang = function (param, paramsub) {
        return getLang('users', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_users').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="usersselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="usersselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
            ...
            {   
                ...
                data: function ( row, type, set, meta ) {
                    ...
                    var text = escapeHtml(row.firstname);
            ...
            {   
                ...
                data: function ( row, type, set, meta ) {
                    ...
                    var text = escapeHtml(row.lastname);
                    ...
                    if(typeof row.email !== 'string'){
                        ...
                        if(row.email.length > 0){
                            return `<a href="javascript:;" 
                                       ...
                                       data-modaldata='{"type":"single", "mailto":"` + escapeHtml(row.email) + `", "id":"` + row.DT_RowId + `", "email_field":"email", "table_name":"users", "jsname":"users"}'
            ...
            {   
                ...
                name: 'gender',
                title: usersLang('gender'),
                data: function ( row, type, set, meta ) {
                
                    var data_list = usersLang('list_gender');
                
                    
                                    
                    return !isEmpty(row.gender)? escapeHtml(data_list[row.gender]) : '';
                ...
                {   
                    sortable : true,
                    name: 'usertype_RELATIONAL_usertype',
                    title: usersLang('usertype'),
                    data: function ( row, type, set, meta ) 
                    {
                        var txt = !isEmpty(row.usertype_RELATIONAL_usertype)? escapeHtml(row.usertype_RELATIONAL_usertype) : '';
                        return '<span title="ID : ' + escapeHtml(row.usertype) + '">' + txt + '</span>';
                     }
                },
                    {
                        name: 'Actions',
                        ...
                        data: function (row) {
                            ...
                            $link += `<a
                               href="` + get_url('showForm/users/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_users"
                               data-modalurl="` + get_url('showForm/users/' + id) + `"
                               data-modaldata='{"user_id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + usersLang('_form_users') + `" 
        ...
        //Form
        $('#form_users').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_users').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return usersLang(param, paramsub);
        },
