...
var c4_email_track = function () {

    var tableID = 'table_c4_email_track';
    var page_slug = 'c4_email_track';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"c4_email_track":"lg"};
    ...
    var get_url = function (param) {
        ...
        return panel_url('c4_email_track' + param);
    ...
    //Module Lang
    var c4_email_trackLang = function (param, paramsub) {
        return getLang('c4_email_track', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_c4_email_track').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="c4_email_trackselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="c4_email_trackselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
                ...
                {   
                    sortable : true,
                    name: 'c4_email_history_RELATIONAL_cid',
                    title: c4_email_trackLang('c4_email_history_id'),
                    data: function ( row, type, set, meta ) 
                    {
                        var txt = !isEmpty(row.c4_email_history_RELATIONAL_cid)? escapeHtml(row.c4_email_history_RELATIONAL_cid) : '';
                        return '<span title="ID : ' + escapeHtml(row.c4_email_history_id) + '">' + txt + '</span>';
                     }
                },
            {   
                sortable : true,
                ...
                name: 'browser',
                title: c4_email_trackLang('browser'),
                data: function ( row, type, set, meta ) {
                    
                    if(typeof row.browser !== 'string'){
                        return '';
                    }
                                        
                    var text = escapeHtml(row.browser);
            ...
            {   
                ...
                name: 'ip',
                title: c4_email_trackLang('ip'),
                data: function ( row, type, set, meta ) {
                    
                    if(typeof row.ip !== 'string'){
                        return '';
                    }
                                        
                    var text = escapeHtml(row.ip);
            ...
            },
                    {
                        ...
                        data: function (row) {
                            var id = row.c4_email_track_id;
                            var $link = ``;

                                var $link =`<div class="btn-group" role="group" aria-label="">`;
        
                            //c4_email_track form button
                            
                            $link += `<a
                               href="` + get_url('showForm/c4_email_track/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_c4_email_track"
                               data-modalurl="` + get_url('showForm/c4_email_track/' + id) + `"
                               data-modaldata='{"c4_email_track_id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + c4_email_trackLang('_form_c4_email_track') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('select', function (e, dt, type, indexes) {
                    ...
                    $('#batch_c4_email_track').collapse('show');
                    if (type === 'row') {
                ...
                .on('deselect', function (e, dt, type, indexes) {
                    ...
                    if (type === 'row') {
                        ...
                        if (count === 0) {
                            $('#batch_c4_email_track').collapse('hide');
                        }
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#c4_email_trackselectAll');
        ...
        //Form
        $('#form_c4_email_track').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_c4_email_track').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return c4_email_trackLang(param, paramsub);
        },
