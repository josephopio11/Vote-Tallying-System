...
var subcounty = function () {

    var tableID = 'table_subcounty';
    var page_slug = 'subcounty';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"subcounty":"lg"};
    ...
    var get_url = function (param) {
        ...
        return panel_url('subcounty' + param);
    ...
    //Module Lang
    var subcountyLang = function (param, paramsub) {
        return getLang('subcounty', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_subcounty').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="subcountyselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="subcountyselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
                ...
                {   
                ...
                },
                    {
                        ...
                        data: function (row) {
                            ...
                            //subcounty form button
                            
                            $link += `<a
                               href="` + get_url('showForm/subcounty/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_subcounty"
                               data-modalurl="` + get_url('showForm/subcounty/' + id) + `"
                               data-modaldata='{"id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + subcountyLang('_form_subcounty') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#subcountyselectAll');
        ...
        //Form
        $('#form_subcounty').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_subcounty').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return subcountyLang(param, paramsub);
        },
