...
var c4_log = function () {
    ...
    var tableID = 'table_c4_log';
    var page_slug = 'c4_log';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"c4_log":"lg"};
    ...
    //Module Lang
    var c4_logLang = function (param, paramsub) {
        return getLang('c4_log', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_c4_log').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="c4_logselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="c4_logselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
            ...
            {   
                ...
                name: 'level',
                title: c4_logLang('level'),
                data: function ( row, type, set, meta ) {
                
                    var data_list = c4_logLang('list_level');
                
                    
                                    
                    return !isEmpty(row.level)? escapeHtml(data_list[row.level]) : '';
            ...
            {   
                ...
                data: function ( row, type, set, meta ) {
                    ...
                    var text = escapeHtml(row.session);
            ...
            {   
                ...
                name: 'get',
                title: c4_logLang('get'),
                data: function ( row, type, set, meta ) {
                    
                    if(typeof row.get !== 'string'){
                        return '';
                    }
                                        
                    var text = escapeHtml(row.get);
                ...
                data: function ( row, type, set, meta ) {
            ...
            },
                    {
                        ...
                        data: function (row) {
                            ...
                            $link += `<a
                               href="` + get_url('showForm/c4_log/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_c4_log"
                               data-modalurl="` + get_url('showForm/c4_log/' + id) + `"
                               data-modaldata='{"c4_log_id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + c4_logLang('_form_c4_log') + `" 
        ...
        //Form
        $('#form_c4_log').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_c4_log').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return c4_logLang(param, paramsub);
        },
