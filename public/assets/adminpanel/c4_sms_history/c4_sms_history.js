...
var c4_sms_history = function () {

    var tableID = 'table_c4_sms_history';
    var page_slug = 'c4_sms_history';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"c4_sms_history":"lg"};
    ...
    var get_url = function (param) {
        ...
        return panel_url('c4_sms_history' + param);
    ...
    //Module Lang
    var c4_sms_historyLang = function (param, paramsub) {
        return getLang('c4_sms_history', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_c4_sms_history').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="c4_sms_historyselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="c4_sms_historyselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
            ...
            {   
                ...
                name: 'smsto',
                title: c4_sms_historyLang('smsto'),
                data: function ( row, type, set, meta ) {
                    
                    if(typeof row.smsto !== 'string'){
                        return '';
                    }
                                        
                    var text = escapeHtml(row.smsto);
            ...
            {   
                ...
                name: 'message',
                title: c4_sms_historyLang('message'),
                data: function ( row, type, set, meta ) {
            ...
            {   
                ...
                name: 'is_sended',
                title: c4_sms_historyLang('is_sended'),
                className: "text-center",
                data: function ( row, type, set, meta ) {
                
                    var data_list = c4_sms_historyLang('list_is_sended');
                ...
                return !isEmpty(row.is_sended)? escapeHtml(data_list[row.is_sended]) : '';
            ...
            },
                    {
                        ...
                        data: function (row) {
                            var id = row.c4_sms_history_id;
                            var $link = ``;

                                var $link =`<div class="btn-group" role="group" aria-label="">`;
        
                            //c4_sms_history form button
                            
                            $link += `<a
                               href="` + get_url('showForm/c4_sms_history/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_c4_sms_history"
                               data-modalurl="` + get_url('showForm/c4_sms_history/' + id) + `"
                               data-modaldata='{"c4_sms_history_id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + c4_sms_historyLang('_form_c4_sms_history') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('select', function (e, dt, type, indexes) {
                    ...
                    $('#batch_c4_sms_history').collapse('show');
                    if (type === 'row') {
                ...
                .on('deselect', function (e, dt, type, indexes) {
                    ...
                    if (type === 'row') {
                        ...
                        if (count === 0) {
                            $('#batch_c4_sms_history').collapse('hide');
                        }
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#c4_sms_historyselectAll');
        ...
        //Form
        $('#form_c4_sms_history').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_c4_sms_history').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return c4_sms_historyLang(param, paramsub);
        },
