...
var c4_session = function () {

    var tableID = 'table_c4_session';
    var page_slug = 'c4_session';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"c4_session":"lg"};
    ...
    var get_url = function (param) {
        ...
        return panel_url('c4_session' + param);
    ...
    //Module Lang
    var c4_sessionLang = function (param, paramsub) {
        return getLang('c4_session', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_c4_session').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="c4_sessionselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="c4_sessionselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
            ...
            {   
                ...
                name: 'ip_address',
                title: c4_sessionLang('ip_address'),
            ...
            {   
                sortable : true,
                name: 'timestamp',
                title: c4_sessionLang('timestamp'),
                data: function ( row, type, set, meta ) {
                    
                    if(!isEmpty(row.timestamp) && typeof row.timestamp === 'string'){
                    
                        var dateX = moment(row.timestamp, 'X');
                        var datenow = moment();
                                            
                                                                    
                        if (dateX > datenow) {
                            return '<span class="text-success" title="' + escapeHtml(row.timestamp) +'">' + moment(row.timestamp, 'X').fromNow() + '</span>';
                        } else {
                            return '<span class="text-danger" title="' + escapeHtml(row.timestamp) +'">'  +  moment(row.timestamp, 'X').fromNow() + '</span>';
                        }
                ...
                data: function ( row, type, set, meta ) {
            ...
            },
                    {
                        ...
                        data: function (row) {
                            ...
                            //c4_session form button
                            
                            $link += `<a
                               href="` + get_url('showForm/c4_session/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_c4_session"
                               data-modalurl="` + get_url('showForm/c4_session/' + id) + `"
                               data-modaldata='{"id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + c4_sessionLang('_form_c4_session') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('select', function (e, dt, type, indexes) {
                    ...
                    $('#batch_c4_session').collapse('show');
                    if (type === 'row') {
                ...
                .on('deselect', function (e, dt, type, indexes) {
                    ...
                    if (type === 'row') {
                        ...
                        if (count === 0) {
                            $('#batch_c4_session').collapse('hide');
                        }
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#c4_sessionselectAll');
        ...
        //Form
        $('#form_c4_session').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_c4_session').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return c4_sessionLang(param, paramsub);
        },
