...
var county = function () {
    ...
    var tableID = 'table_county';
    var page_slug = 'county';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"county":"lg"};
    ...
    //Module Lang
    var countyLang = function (param, paramsub) {
        return getLang('county', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_county').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="countyselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="countyselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
                ...
                {   
                ...
                },
                    {
                        ...
                        data: function (row) {
                            ...
                            $link += `<a
                               href="` + get_url('showForm/county/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_county"
                               data-modalurl="` + get_url('showForm/county/' + id) + `"
                               data-modaldata='{"countyid":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + countyLang('_form_county') + `" 
        ...
        //Form
        $('#form_county').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_county').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return countyLang(param, paramsub);
        },
