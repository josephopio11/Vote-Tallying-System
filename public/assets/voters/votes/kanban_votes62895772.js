...
var id =  data.DT_RowId;
                            ...
                            $link += `<a
                               ...
                               data-modalsize="lg"
                               data-datatable="table_votes62895772"
                            ...
                            $link += `<a
                               ...
                               > <i class="fas fa-file-pdf"></i>  ` + getLang('votes','_form_votes62729055') + ' ' +  getLang('votes',('pdf') + `
                                ...
                                $link += `    
                                   <a
                                    href="` + panel_url('votes/delete/' + id) + `" 
                                    data-actionurl="` + panel_url('votes/delete/' + id) + `"
...
function loadKanbanGroup(kanbangroup, activepage) {
        ...
        $.ajax({
            url: panel_url('votes/readVotes/votes62895772/'),
            dataType: "json",
            async: false,
            cache: true,
            data: {
            start: start,
            length: PAGE_LENGT,
            formFilter: $('#form_votes62895772').serialize() + '&totalvoters=' + kanbangroup,
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#form_votes62895772').find('.generalSearch').bind("keyup search", function () {

        loadAllKanbanGroup();
    });


    $('#form_votes62895772').find('.formSearch').on('change', function () {
