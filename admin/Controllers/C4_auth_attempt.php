...
use Admin\Models\C4_auth_attemptModel;

class C4_auth_attempt extends BaseController
{
    ...
    {
        helper(['Admin\c4_auth_attempt']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = authentication_attempt 
     * authentication_attempt   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '387e488']);

        $data                 = ['page_title' => lang('c4_auth_attempt._page_authentication_attempt')];
        $data['breadcrumb'][] = ['url' => admin_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('c4_auth_attempt._page_authentication_attempt'), 'class' => 'active'];

        $this->_render('authentication_attempt', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = authentication_attempt
     * Authentication Attempt  
     */
    public function authentication_attempt()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '81cf23f']);
     ...
     * @return json
     */
    public function save($form_slug, $c4_auth_attempt_id = '')
    {
        $data       = $this->request->getPost();
        $validation = \Config\Services::validation();

        //---------------------------------------------------------------------//
        //For form_slug=c4_auth_attempt pageSlug=authentication_attempt
        if($form_slug === 'c4_auth_attempt')
        {

            // c4_auth_attempt form Validations and security
            $validation->setRules([
                'ip_address'    => ['label' => lang('c4_auth_attempt.ip_address'), 'rules'=>"required|valid_ip[]|max_length[96]"],
                'email'         => ['label' => lang('c4_auth_attempt.email'), 'rules'=>"required|valid_email|max_length[128]"],
                'table'         => ['label' => lang('c4_auth_attempt.table'), 'rules'=>"required|max_length[128]|alpha_numeric_space_turkish"],
                'whoisID'       => ['label' => lang('c4_auth_attempt.whoisID'), 'rules'=>"required|integer|max_length[20]"],
                'attemp_type'   => ['label' => lang('c4_auth_attempt.attemp_type'), 'rules'=>"required|max_length[128]|alpha_numeric_space_turkish"],
                'success'       => ['label' => lang('c4_auth_attempt.success'), 'rules'=>"required|integer|max_length[1]|in_list[0,1]"],
                'message'       => ['label' => lang('c4_auth_attempt.message'), 'rules'=>"required|max_length[255]|alpha_numeric_space_turkish"],
                'userAgent'     => ['label' => lang('c4_auth_attempt.userAgent'), 'rules'=>"required|max_length[255]|alpha_numeric_space_turkish"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['ip_address', 'email', 'table', 'whoisID', 'attemp_type', 'success', 'message', 'userAgent', 'c4_auth_attempt_id'];
        ...
        $C4_auth_attemptModel = new C4_auth_attemptModel();

        try
        {
            $C4_auth_attemptModel->save($data);
        }
        catch (\Exception $e)
        {
        ...
        if(empty($c4_auth_attempt_id))
        {
            $c4_auth_attempt_id = $C4_auth_attemptModel->getInsertID();
        }

        return $this->respondCreated(['id' => $c4_auth_attempt_id, 'message' => lang('home.saved')]);
    }
 
    //--------------------------------------------------------------------

    /**
     * Read c4_auth_attempt data
     ...
     * 
     */
    public function readC4_auth_attempt($pageSlug)
    {

        $pageList = ['authentication_attempt'];
        ...
        $model      = new C4_auth_attemptModel();
        ...
        //--------------------------------------------------------------------//
        // Page authentication_attempt
        //--------------------------------------------------------------------//

        if($pageSlug === 'authentication_attempt')
        {
            $select_text       = "c4_auth_attempt.*";
            $multipleFields    = [];
            ...
            if (!empty($search_text))
            {
                ...
                $model->groupStart();
                $model->orLike('c4_auth_attempt.ip_address', $search_text);
                $model->groupEnd();
            }
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("c4_auth_attempt.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        {
            $model->where('c4_auth_attempt.deleted_at', NULL);
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("c4_auth_attempt.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("c4_auth_attempt.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $C4_auth_attemptModel = new C4_auth_attemptModel();
        if ($C4_auth_attemptModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID C4_auth_attemptModel Delete Error");
            return $this->fail($C4_auth_attemptModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('c4_auth_attempt._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'c4_auth_attempt')
        {
            $data['breadcrumb'][] = ['url' => admin_url('c4_auth_attempt/authentication_attempt'), 'title' => lang('c4_auth_attempt._page_authentication_attempt')];
            $data['breadcrumb'][] = ['title' => lang('c4_auth_attempt._form_c4_auth_attempt'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $C4_auth_attemptModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $C4_auth_attemptModel->find($copy);
            
            if(isset($data['formData']['c4_auth_attempt_id']))
            {
                unset($data['formData']['c4_auth_attempt_id']);
            }
        }
    ...
    //--------------------------------------------------------------------
    /**
     * Update single field of c4_auth_attempt 
     * 
     * @param int $id
     * @return mix
     */
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'd76f961']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'admin/Language/' . $locale . '/c4_auth_attempt.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/c4_auth_attempt.php';
        ...
        echo 'var LANG_c4_auth_attempt = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'c4_auth_attempt';

        if ($this->request->isAJAX())
        {
            try
            {
                echo admin_view('c4_auth_attempt/' . $page, $data);
            }
            catch (\Exception $e)
            {
