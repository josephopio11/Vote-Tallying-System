...
class C4_zone extends BaseController
{
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '6359aa1']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'e22a4a3']);
    ...
    public function save($form_slug, $c4_zone_id = '')
    {
        ...
        if($form_slug === 'c4_zone')
        {
            ...
            $validation->setRules([
                ...
                'name'          => ['label' => lang('c4_zone.name'), 'rules'=>"required|alpha_numeric_space_turkish|max_length[128]"],
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'c4_zone')
        {
            $data['breadcrumb'][] = ['url' => admin_url('c4_zone/c4_zone'), 'title' => lang('c4_zone._page_c4_zone')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '9e5026c']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'admin/Language/' . $locale . '/c4_zone.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/c4_zone.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo admin_view('c4_zone/' . $page, $data);
            }
            catch (\Exception $e)
            {
