...
use Admin\Models\C4_sessionModel;

class C4_session extends BaseController
{
    ...
    {
        helper(['Admin\c4_session']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = c4_session 
     * c4_session   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'd51b218']);

        $data                 = ['page_title' => lang('c4_session._page_c4_session')];
        $data['breadcrumb'][] = ['url' => admin_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('c4_session._page_c4_session'), 'class' => 'active'];

        $this->_render('c4_session', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = c4_session
     * Session  
     */
    public function c4_session()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'c6726ea']);
    ...
    public function save($form_slug, $id = '')
    {
        ...
        //---------------------------------------------------------------------//
        //For form_slug=c4_session pageSlug=c4_session
        if($form_slug === 'c4_session')
        {

            // c4_session form Validations and security
            $validation->setRules([
                'ip_address'    => ['label' => lang('c4_session.ip_address'), 'rules'=>"required|valid_ip[]|max_length[45]"],
                'timestamp'     => ['label' => lang('c4_session.timestamp'), 'rules'=>"required|integer|max_length[10]"],
                'data'          => ['label' => lang('c4_session.data'), 'rules'=>"permit_empty|string"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['ip_address', 'timestamp', 'data', 'id'];
        ...
        $C4_sessionModel = new C4_sessionModel();

        try
        {
            $C4_sessionModel->save($data);
        }
        catch (\Exception $e)
        {
        ...
        if(empty($id))
        {
            $id = $C4_sessionModel->getInsertID();
        }
     ...
     * 
     */
    public function readC4_session($pageSlug)
    {
        ...
        if($pageSlug === 'c4_session')
        {
            $select_text       = "c4_session.*";
            $multipleFields    = [];
            ...
            if (!empty($search_text))
            {
                ...
                $model->groupStart();
                $model->orLike('c4_session.ip_address', $search_text);
                $model->groupEnd();
            }
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("c4_session.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("c4_session.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("c4_session.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $C4_sessionModel = new C4_sessionModel();
        $C4_sessionModel->where('id', filter_var($id, FILTER_SANITIZE_STRING));
        if ($C4_sessionModel->delete($id, true) === false)
        {
            log_message('error', "Error: $id ID C4_sessionModel Delete Error");
            return $this->fail($C4_sessionModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('c4_session._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'c4_session')
        {
            $data['breadcrumb'][] = ['url' => admin_url('c4_session/c4_session'), 'title' => lang('c4_session._page_c4_session')];
            $data['breadcrumb'][] = ['title' => lang('c4_session._form_c4_session'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $C4_sessionModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $C4_sessionModel->find($copy);
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '1ff33af']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'admin/Language/' . $locale . '/c4_session.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/c4_session.php';
        ...
        echo 'var LANG_c4_session = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'c4_session';

        if ($this->request->isAJAX())
        {
            try
            {
                echo admin_view('c4_session/' . $page, $data);
            }
            catch (\Exception $e)
            {
