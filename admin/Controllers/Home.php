...
class Home extends BaseController
{
    ...
    public function accountsave($form_slug, $admin_id)
    {
        ...
        $data['admin_id'] = $authService->getId();
        ...
        if ($form_slug === 'personel')
        {
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['avatar', 'firstname', 'lastname', 'admin_id'];
        ...
        elseif ($form_slug === 'change_password')
        {
            ...
            // clear undefined post fields for security..
            //----------------------------------------------------------------//
            $filterFormFields = ['current_password', 'new_password', 'new_password_confirm', 'admin_id'];
            $data             = array_intersect_key($data, array_flip($filterFormFields));
            //----------------------------------------------------------------//
    ...
    public function sendEmail($id = null)
    {
        ...
        elseif ($type === 'multiple')
        {
            ...
            if (!empty($tableData))
            {
                ...
                $tableData['HTTP_PANEL_URL'] = admin_url();
    ...
    private function _render($fileName, $data = [])
    {
        ...
        {
            echo admin_view('home/' . $fileName, $data);
        }
        else
        {
    ...
    {
        //Cache
        $this->response->setCache(['max-age'  => 300, 's-maxage' => 900, 'etag' => '96d11fe']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $homeLangFile = ROOTPATH . 'admin/Language/' . $locale . '/home.php';

        if (!file_exists($homeLangFile))
        {
            //Check default lang file
            $homeLangFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/home.php';
        ...
        $langArray['panel_url'] = site_url("admin/$locale");
    ...
    public function showCalendar($calendarSlug)
    {
        $calendarSlug = trim($calendarSlug);
        $calendarList = ['admin_panel_calendar'];

        if (!in_array($calendarSlug, $calendarList))
        {
            return $this->failNotFound("$calendarSlug form not founded");
        }

        $data                 = ['page_title' => lang("home._calendar_{$calendarSlug}")];
        $data['breadcrumb'][] = ['url' => admin_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang("home._calendar_{$calendarSlug}"), 'class' => 'active'];
        
        $this->_render('calendar/' . $calendarSlug, $data);
    }
