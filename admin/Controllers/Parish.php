...
class Parish extends BaseController
{
    ...
    {
        helper(['Admin\parish', 'Admin\subcounty']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'dd0513f']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '846d6e1']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'parish')
        {
            $data['breadcrumb'][] = ['url' => admin_url('parish/parish'), 'title' => lang('parish._page_parish')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '075aecb']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'admin/Language/' . $locale . '/parish.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/parish.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo admin_view('parish/' . $page, $data);
            }
            catch (\Exception $e)
            {
