...
use Admin\Models\C4_sms_historyModel;

class C4_sms_history extends BaseController
{
    ...
    {
        helper(['Admin\c4_sms_history']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = c4_sms_history 
     * c4_sms_history   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '382a7fc']);

        $data                 = ['page_title' => lang('c4_sms_history._page_c4_sms_history')];
        $data['breadcrumb'][] = ['url' => admin_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('c4_sms_history._page_c4_sms_history'), 'class' => 'active'];

        $this->_render('c4_sms_history', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = c4_sms_history
     * Sms History  
     */
    public function c4_sms_history()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '18dd03c']);
     ...
     * @return json
     */
    public function save($form_slug, $c4_sms_history_id = '')
    {
        $data       = $this->request->getPost();
        $validation = \Config\Services::validation();

        //---------------------------------------------------------------------//
        //For form_slug=c4_sms_history pageSlug=c4_sms_history
        if($form_slug === 'c4_sms_history')
        {

            // c4_sms_history form Validations and security
            $validation->setRules([
                'smsto'         => ['label' => lang('c4_sms_history.smsto'), 'rules'=>"required|max_length[256]|alpha_numeric_space_turkish"],
                'message'       => ['label' => lang('c4_sms_history.message'), 'rules'=>"required|alpha_numeric_space_turkish"],
                'is_sended'     => ['label' => lang('c4_sms_history.is_sended'), 'rules'=>"required|integer|max_length[1]|in_list[0,1]"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['smsto', 'message', 'is_sended', 'c4_sms_history_id'];
        ...
        $C4_sms_historyModel = new C4_sms_historyModel();

        try
        {
            $C4_sms_historyModel->save($data);
        }
        catch (\Exception $e)
        {
        ...
        if(empty($c4_sms_history_id))
        {
            $c4_sms_history_id = $C4_sms_historyModel->getInsertID();
        }

        return $this->respondCreated(['id' => $c4_sms_history_id, 'message' => lang('home.saved')]);
    }
 
    //--------------------------------------------------------------------

    /**
     * Read c4_sms_history data
     ...
     * 
     */
    public function readC4_sms_history($pageSlug)
    {

        $pageList = ['c4_sms_history'];
        ...
        if($pageSlug === 'c4_sms_history')
        {
            $select_text       = "c4_sms_history.*";
            $multipleFields    = [];
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("c4_sms_history.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        {
            $model->where('c4_sms_history.deleted_at', NULL);
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("c4_sms_history.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("c4_sms_history.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $C4_sms_historyModel = new C4_sms_historyModel();
        if ($C4_sms_historyModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID C4_sms_historyModel Delete Error");
            return $this->fail($C4_sms_historyModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('c4_sms_history._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'c4_sms_history')
        {
            $data['breadcrumb'][] = ['url' => admin_url('c4_sms_history/c4_sms_history'), 'title' => lang('c4_sms_history._page_c4_sms_history')];
            $data['breadcrumb'][] = ['title' => lang('c4_sms_history._form_c4_sms_history'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $C4_sms_historyModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $C4_sms_historyModel->find($copy);
            
            if(isset($data['formData']['c4_sms_history_id']))
            {
                unset($data['formData']['c4_sms_history_id']);
            }
        }
    ...
    //--------------------------------------------------------------------
    /**
     * Update single field of c4_sms_history 
     * 
     * @param int $id
     * @return mix
     */
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '735c363']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'admin/Language/' . $locale . '/c4_sms_history.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/c4_sms_history.php';
        ...
        echo 'var LANG_c4_sms_history = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'c4_sms_history';

        if ($this->request->isAJAX())
        {
            try
            {
                echo admin_view('c4_sms_history/' . $page, $data);
            }
            catch (\Exception $e)
            {
