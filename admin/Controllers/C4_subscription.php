...
use Admin\Models\C4_subscriptionModel;
...
class C4_subscription extends BaseController
{
    ...
    {
        helper(['Admin\c4_subscription', 'Admin\company']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '3dec79a']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '0597915']);
    ...
    public function save($form_slug, $c4_subscription_id = '')
    {
        ...
        if($form_slug === 'c4_subscription')
        {
            ...
            $validation->setRules([
                ...
                'plan_duration' => ['label' => lang('c4_subscription.plan_duration'), 'rules'=>"required|max_length[256]|alpha_numeric_space_turkish"],
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['company_id', 'subscription_status', 'due_date', 'plan_duration', 'trail_start_date', 'trial_end_date', 'subscription_start_date', 'subscription_end_date', 'c4_subscription_id'];
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        {
            $data['breadcrumb'][] = ['url' => admin_url('c4_subscription/c4_subscription'), 'title' => lang('c4_subscription._page_c4_subscription')];
    ...
    //--------------------------------------------------------------------
    /**
     * Update single field of c4_subscription 
     * 
     * @param int $id
     * @return mix
     */
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '89e8bf1']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'admin/Language/' . $locale . '/c4_subscription.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'admin/Language/' . $conf->defaultLocale . '/c4_subscription.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo admin_view('c4_subscription/' . $page, $data);
            }
            catch (\Exception $e)
            {
