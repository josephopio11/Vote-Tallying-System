...
{
    function admin_url($param = NULL, $locale = NULL)
    {
        if (empty($locale))
        {
        ...
        return site_url("admin/$locale" . '/' . $param);
    }
...
{
    function admin_assets($param = NULL)
    {
        return site_url('assets/admin/'. $param);
    }
}
...
{
    function admin_view(string $name, array $data = [], array $options = []): string
    {
        $options['debug'] = false;

        return view('Admin\\' . $name, $data, $options);
    }
...
if (!function_exists('getTemplateByName'))
{
    function getTemplateByName($templateName, $renderData = [])
    {
        ...
        $C4_templateModel = new \Admin\Models\C4_templateModel();
