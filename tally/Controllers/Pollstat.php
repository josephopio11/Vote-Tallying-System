...
use Tally\Models\PollstatModel;
use Tally\Models\ParishModel;
...
class Pollstat extends BaseController
{
    ...
    {
        helper(['Tally\pollstat', 'Tally\parish']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'e52aa7b']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'e1d6dac']);
    ...
    public function save($form_slug, $id = '')
    {
        ...
        if($form_slug === 'pollstat')
        {
            ...
            $validation->setRules([
                ...
                'name'          => ['label' => lang('pollstat.name'), 'rules'=>"required|string|max_length[50]"],
            ]);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        {
            $data['breadcrumb'][] = ['url' => tally_url('pollstat/pollstat'), 'title' => lang('pollstat._page_pollstat')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '52caffc']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'tally/Language/' . $locale . '/pollstat.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/pollstat.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('pollstat/' . $page, $data);
            }
            catch (\Exception $e)
            {
