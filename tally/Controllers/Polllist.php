...
use Tally\Models\PolllistModel;
...
class Polllist extends BaseController
{
    ...
    {
        helper(['Tally\polllist', 'Tally\c4_zone', 'Tally\county', 'Tally\subcounty', 'Tally\parish', 'Tally\pollstat']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'f208952']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '6f47e4d']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        {
            $data['breadcrumb'][] = ['url' => tally_url('polllist/polllist'), 'title' => lang('polllist._page_polllist')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'e8edfee']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'tally/Language/' . $locale . '/polllist.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/polllist.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('polllist/' . $page, $data);
            }
            catch (\Exception $e)
            {
