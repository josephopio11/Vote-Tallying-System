...
class County extends BaseController
{
    ...
    {
        helper(['Tally\county', 'Tally\c4_zone']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '8a89b39']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '8d16511']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'county')
        {
            $data['breadcrumb'][] = ['url' => tally_url('county/county'), 'title' => lang('county._page_county')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'c459c9f']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'tally/Language/' . $locale . '/county.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/county.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('county/' . $page, $data);
            }
            catch (\Exception $e)
            {
