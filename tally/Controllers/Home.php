...
class Home extends BaseController
{
    ...
    public function accountsave($form_slug, $user_id)
    {
        ...
        $UserModel = new \Tally\Models\UserModel();
    ...
    public function sendEmail($id = null)
    {
        ...
        elseif ($type === 'multiple')
        {
            ...
            if (!empty($tableData))
            {
                ...
                $tableData['HTTP_PANEL_URL'] = tally_url();
    ...
    private function _render($fileName, $data = [])
    {
        ...
        {
            echo tally_view('home/' . $fileName, $data);
        }
        else
        {
    ...
    {
        //Cache
        $this->response->setCache(['max-age'  => 300, 's-maxage' => 900, 'etag' => '73fd4cd']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $homeLangFile = ROOTPATH . 'tally/Language/' . $locale . '/home.php';

        if (!file_exists($homeLangFile))
        {
            //Check default lang file
            $homeLangFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/home.php';
        ...
        $langArray['panel_url'] = site_url("tally/$locale");
