...
class Parish extends BaseController
{
    ...
    {
        helper(['Tally\parish', 'Tally\subcounty']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '08eb3a5']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '906fe9f']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'parish')
        {
            $data['breadcrumb'][] = ['url' => tally_url('parish/parish'), 'title' => lang('parish._page_parish')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '2cd9df6']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'tally/Language/' . $locale . '/parish.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/parish.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('parish/' . $page, $data);
            }
            catch (\Exception $e)
            {
