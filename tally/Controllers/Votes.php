...
class Votes extends BaseController
{
    ...
    {
        helper(['Tally\votes', 'Tally\c4_zone', 'Tally\county', 'Tally\subcounty', 'Tally\parish', 'Tally\pollstat', 'Tally\users']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'fbc10c1']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '3e85593']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'votes')
        {
            $data['breadcrumb'][] = ['url' => tally_url('votes/votes'), 'title' => lang('votes._page_votes')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '5db7227']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'tally/Language/' . $locale . '/votes.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/votes.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('votes/' . $page, $data);
            }
            catch (\Exception $e)
            {
