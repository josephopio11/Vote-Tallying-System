...
use Tally\Models\AuthenticationModel;
use Tally\Controllers\BaseController;
...
class Authentication extends BaseController
{
    ...
    public function login() 
    {
        ...
        $authConfig = new \Tally\Config\Authentication();
        ...
        if ($authService->set_login($memberData, intval($remember), 'std'))
        {
            ...
            if ($subscriptionService->check() === false)
            {
                return $this->fail(['redirectURL' => $subscriptionService->getPage(), 'error' => [$subscriptionService->getErrorString()]]);
            }

            $goBack = empty($goBack) ? tally_url('home/index') : site_url($goBack);
    ...
    public function register()
    {
        ...
        // -------------------------------------------------------------------// 
        //SAVE for Subscription Table : company        // We use standart query db  to avoiding staticDBField
        // -------------------------------------------------------------------// 
        $companyData['name'] = $lastname;
        $companyData['country_id'] = $country_id;
        $companyData['zone_id'] = $zone_id;
        ...
        $memberData['company_id'] = $company_id;
        // -------------------------------------------------------------------//
        ...
        $renderData = [
            ...
            'PASSWORD'  => $password,
            'LINK'      => tally_url('auth/login'),
        ];

        sendEmailTemplate('mailNewRegister', $email, $renderData);
        
        return $this->respond(['redirectURL' => tally_url('auth/login'), 'waitTime' => 5000, 'messages' => [lang('auth.yourPasswordWillSent')]], 202);
    ...
    public function forgot() 
    {
        ...
        $renderData = [
            ...
            'LASTNAME' => $memberData['lastname'],
            'CODE' => $code,
            'LINK' => tally_url('auth/resetpassword') . "?code=$code&id=" . $memberData['user_id']
        ];
    ...
    //--------------------------------------------------------------------    
     ...
     private function _render($page, $data=[]) 
     {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('authentication/' . $page, $data);
            }
            catch (\Exception $e)
            {
        ...
        else
        {
            try
            {
                echo tally_view('themes/' . $this->theme . '/authencationHeader', $data);
                echo tally_view('authentication/' . $page, $data);
                echo tally_view('themes/' . $this->theme . '/authencationFooter', $data);
            }
            catch (\Exception $e)
            {
                log_message('alert', '[ERROR] {exception}', ['exception' => $e]);
