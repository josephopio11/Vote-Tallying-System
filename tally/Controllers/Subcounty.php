...
class Subcounty extends BaseController
{
    ...
    {
        helper(['Tally\subcounty', 'Tally\county']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'a9a5478']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'f6154dc']);
    ...
    public function save($form_slug, $id = '')
    {
        ...
        if($form_slug === 'subcounty')
        {
            ...
            $validation->setRules([
                'countyid'      => ['label' => lang('subcounty.countyid'), 'rules'=>"permit_empty|integer|max_length[11]"],
                'name'          => ['label' => lang('subcounty.name'), 'rules'=>"permit_empty|string|max_length[25]"],
            ]);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        {
            $data['breadcrumb'][] = ['url' => tally_url('subcounty/subcounty'), 'title' => lang('subcounty._page_subcounty')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'deaab12']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'tally/Language/' . $locale . '/subcounty.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'tally/Language/' . $conf->defaultLocale . '/subcounty.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo tally_view('subcounty/' . $page, $data);
            }
            catch (\Exception $e)
            {
