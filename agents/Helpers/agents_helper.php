 ...
 * 
 */
if (!function_exists('agents_url'))
{
    function agents_url($param = NULL, $locale = NULL)
    {
        if (empty($locale))
        {
        ...
        return site_url("agents/$locale" . '/' . $param);
    }
...
if (!function_exists('agents_assets'))
{
    function agents_assets($param = NULL)
    {
        return site_url('assets/agents/'. $param);
    }
}

//--------------------------------------------------------------------

if (!function_exists('agents_view'))
{
    function agents_view(string $name, array $data = [], array $options = []): string
    {
        $options['debug'] = false;

        return view('Agents\\' . $name, $data, $options);
    }
...
if (!function_exists('getTemplateByName'))
{
    function getTemplateByName($templateName, $renderData = [])
    {
        ...
        $C4_templateModel = new \Agents\Models\C4_templateModel();
