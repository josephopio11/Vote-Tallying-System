...
use Agents\Models\SubscriptionModel;
use Agents\Config\SubscriptionConfig;
...
class SubscriptionService extends BaseService
{
    ...
    public function check(): bool
    {
        ...
        $this->session->set('agentsDueDate', $due_date);
        $this->session->set('agentsStatus', $subscription_status);
    ...
    public function updateStatus($subscriptionStatus, $duration = null): bool
    {
        ...
        if (!empty($duration))
        {
            ...
            $this->session->set('agentsDueDate', $newDueDate);
        }
        ...
        $this->session->set('agentsStatus', $subscriptionStatus);
    ...
    {
        if (!$this->session->has('agentsStatus'))
        {
            $this->check();
        }

        return $this->session->get('agentsStatus');
    }
    ...
    {
        if (!$this->session->has('agentsDueDate'))
        {
            $this->check();
        }

        return $this->session->get('agentsDueDate');
    }
    ...
    private function startTrail()
    {
        ...
        $this->session->set('agentsDueDate', $due_date);
        $this->session->set('agentsStatus', 'in_trial');
        $this->c4_subscription_id = $subscriptionData['c4_subscription_id'];
    ...
    {
        return agents_url('subscription/index');
    }
