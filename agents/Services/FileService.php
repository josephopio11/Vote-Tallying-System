...
use Agents\Models\C4_fileModel;
...
class FileService extends BaseService
{
    ...
    function upload($folderName = '', $isPublic = null)
    {
        ...
        if ($file->isValid() && !$file->hasMoved())
        {
            ...
            if (!$this->getIsPublic())
            {
                $downloadUrl = agents_url("general/downloadfile/$file_id");
            }

            $file_data['file_id']     = $file_id;
            $file_data['downloadUrl'] = $downloadUrl;
            $file_data['icon_url']    = $icon_url;
            $file_data['deleteUrl']   = agents_url(explode('/', $folderName)[0] . "/deleteFile/$file_id");
    ...
    public function analyzeFiledata($fileDBData)
    {
        ...
        {
            $fileDBData['url_download'] = agents_url('home/downloadfile/' . DIRECTORY_SEPARATOR . $fileDBData['file_id']);
