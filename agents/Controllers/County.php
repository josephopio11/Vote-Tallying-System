...
class County extends BaseController
{
    ...
    {
        helper(['Agents\county', 'Agents\c4_zone']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'd930e17']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '2fd22ce']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'county')
        {
            $data['breadcrumb'][] = ['url' => agents_url('county/county'), 'title' => lang('county._page_county')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '26e746b']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'agents/Language/' . $locale . '/county.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'agents/Language/' . $conf->defaultLocale . '/county.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo agents_view('county/' . $page, $data);
            }
            catch (\Exception $e)
            {
