...
use Agents\Models\C4_countryModel;
...
class C4_zone extends BaseController
{
    ...
    {
        helper(['Agents\c4_zone', 'Agents\c4_country']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '2fe9360']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'ef09768']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'c4_zone')
        {
            $data['breadcrumb'][] = ['url' => agents_url('c4_zone/c4_zone'), 'title' => lang('c4_zone._page_c4_zone')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '84e517e']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'agents/Language/' . $locale . '/c4_zone.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'agents/Language/' . $conf->defaultLocale . '/c4_zone.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo agents_view('c4_zone/' . $page, $data);
            }
            catch (\Exception $e)
            {
