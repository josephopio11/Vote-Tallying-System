...
use Agents\Models\AuthenticationModel;
use Agents\Controllers\BaseController;
...
class Authentication extends BaseController
{
    ...
    public function login() 
    {
        ...
        $authConfig = new \Agents\Config\Authentication();
        ...
        if ($authService->set_login($memberData, intval($remember), 'std'))
        {
            ...
            $goBack = empty($goBack) ? agents_url('home/index') : site_url($goBack);
    ...
    public function register()
    {
        ...
        $renderData = [
            ...
            'PASSWORD'  => $password,
            'LINK'      => agents_url('auth/login'),
        ];

        sendEmailTemplate('mailNewRegister', $email, $renderData);
        
        return $this->respond(['redirectURL' => agents_url('auth/login'), 'waitTime' => 5000, 'messages' => [lang('auth.yourPasswordWillSent')]], 202);
    ...
    public function forgot() 
    {
        ...
        $renderData = [
            ...
            'LASTNAME' => $memberData['lastname'],
            'CODE' => $code,
            'LINK' => agents_url('auth/resetpassword') . "?code=$code&id=" . $memberData['user_id']
        ];
    ...
    //--------------------------------------------------------------------    
     ...
     private function _render($page, $data=[]) 
     {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo agents_view('authentication/' . $page, $data);
            }
            catch (\Exception $e)
            {
        ...
        else
        {
            try
            {
                echo agents_view('themes/' . $this->theme . '/authencationHeader', $data);
                echo agents_view('authentication/' . $page, $data);
                echo agents_view('themes/' . $this->theme . '/authencationFooter', $data);
            }
            catch (\Exception $e)
            {
                log_message('alert', '[ERROR] {exception}', ['exception' => $e]);
