...
class Pollstat extends BaseController
{
    ...
    {
        helper(['Agents\pollstat', 'Agents\parish']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'a248f81']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'cb3ed6e']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        {
            $data['breadcrumb'][] = ['url' => agents_url('pollstat/pollstat'), 'title' => lang('pollstat._page_pollstat')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '310e66b']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'agents/Language/' . $locale . '/pollstat.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'agents/Language/' . $conf->defaultLocale . '/pollstat.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo agents_view('pollstat/' . $page, $data);
            }
            catch (\Exception $e)
            {
