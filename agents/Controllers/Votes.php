...
class Votes extends BaseController
{
    ...
    {
        helper(['Agents\votes', 'Agents\c4_zone', 'Agents\county', 'Agents\subcounty', 'Agents\parish', 'Agents\pollstat', 'Agents\users']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '377ce0d']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '83c94f1']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'votes')
        {
            $data['breadcrumb'][] = ['url' => agents_url('votes/votes'), 'title' => lang('votes._page_votes')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'ef8d09a']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'agents/Language/' . $locale . '/votes.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'agents/Language/' . $conf->defaultLocale . '/votes.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo agents_view('votes/' . $page, $data);
            }
            catch (\Exception $e)
            {
