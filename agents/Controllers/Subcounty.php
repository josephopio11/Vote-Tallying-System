...
class Subcounty extends BaseController
{
    ...
    {
        helper(['Agents\subcounty', 'Agents\county']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '85825f3']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '980d110']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        {
            $data['breadcrumb'][] = ['url' => agents_url('subcounty/subcounty'), 'title' => lang('subcounty._page_subcounty')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '3a76657']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'agents/Language/' . $locale . '/subcounty.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'agents/Language/' . $conf->defaultLocale . '/subcounty.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo agents_view('subcounty/' . $page, $data);
            }
            catch (\Exception $e)
            {
