 ...
 * 
 */
if (!function_exists('voters_url'))
{
    function voters_url($param = NULL, $locale = NULL)
    {
        if (empty($locale))
        {
        ...
        return site_url("voters/$locale" . '/' . $param);
    }
...
if (!function_exists('voters_assets'))
{
    function voters_assets($param = NULL)
    {
        return site_url('assets/voters/'. $param);
    }
}

//--------------------------------------------------------------------

if (!function_exists('voters_view'))
{
    function voters_view(string $name, array $data = [], array $options = []): string
    {
        $options['debug'] = false;

        return view('Voters\\' . $name, $data, $options);
    }
...
if (!function_exists('getTemplateByName'))
{
    function getTemplateByName($templateName, $renderData = [])
    {
        ...
        $C4_templateModel = new \Voters\Models\C4_templateModel();
