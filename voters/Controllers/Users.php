...
class Users extends BaseController
{
    ...
    {
        helper(['Voters\users', 'Voters\usertype']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '494f3ec']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '854aa42']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'users')
        {
            $data['breadcrumb'][] = ['url' => voters_url('users/users'), 'title' => lang('users._page_users')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'cf717df']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'voters/Language/' . $locale . '/users.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/users.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo voters_view('users/' . $page, $data);
            }
            catch (\Exception $e)
            {
