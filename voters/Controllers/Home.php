...
class Home extends BaseController
{
    ...
    public function accountsave($form_slug, $user_id)
    {
        ...
        $UsersModel = new \Voters\Models\UsersModel();
    ...
    public function sendEmail($id = null)
    {
        ...
        elseif ($type === 'multiple')
        {
            ...
            if (!empty($tableData))
            {
                ...
                $tableData['HTTP_PANEL_URL'] = voters_url();
    ...
    private function _render($fileName, $data = [])
    {
        ...
        {
            echo voters_view('home/' . $fileName, $data);
        }
        else
        {
    ...
    {
        //Cache
        $this->response->setCache(['max-age'  => 300, 's-maxage' => 900, 'etag' => 'd9bc68b']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $homeLangFile = ROOTPATH . 'voters/Language/' . $locale . '/home.php';

        if (!file_exists($homeLangFile))
        {
            //Check default lang file
            $homeLangFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/home.php';
        ...
        $langArray['panel_url'] = site_url("voters/$locale");
