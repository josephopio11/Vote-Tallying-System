...
use Voters\Models\PollstatModel;
use Voters\Models\ParishModel;

class Pollstat extends BaseController
{
    ...
    {
        helper(['Voters\pollstat', 'Voters\parish']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = pollstat 
     * pollstat   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'f40761c']);

        $data                 = ['page_title' => lang('pollstat._page_pollstat')];
        $data['breadcrumb'][] = ['url' => voters_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('pollstat._page_pollstat'), 'class' => 'active'];

        $this->_render('pollstat', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = pollstat
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '7ebb048']);
    ...
    public function save($form_slug, $id = '')
    {
        ...
        //---------------------------------------------------------------------//
        //For form_slug=pollstat pageSlug=pollstat
        if($form_slug === 'pollstat')
        {

            // pollstat form Validations and security
            $validation->setRules([
                'parishid'      => ['label' => lang('pollstat.parishid'), 'rules'=>"required|integer|max_length[11]"],
                'name'          => ['label' => lang('pollstat.name'), 'rules'=>"required|string|max_length[25]"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['parishid', 'name', 'id'];
        ...
        $PollstatModel = new PollstatModel();

        try
        {
            $PollstatModel->save($data);
        }
        catch (\Exception $e)
        {
     ...
     * 
     */
    public function readPollstat($pageSlug)
    {
        ...
        if($pageSlug === 'pollstat')
        {
            $select_text       = "pollstat.*, parish.name as parish_RELATIONAL_name";
            $multipleFields    = [];

            //------------------------------------------------------------------//
            //merge relation fields names to make Sortable
            $table_colons = array_merge($table_colons, ['parish_RELATIONAL_name']);
            //------------------------------------------------------------------//
            //------------------------------------------------------------------//        
            // Left Join Text
            //------------------------------------------------------------------//
            $model->join("(SELECT id, name FROM parish WHERE parish.deleted_at IS NULL AND parish.status = '1' GROUP BY id ORDER BY id DESC) parish", 'parish.id = pollstat.parishid', 'left');

            if (!empty($search_text))
            {
                $search_text = filter_var($search_text, FILTER_SANITIZE_STRING);
                $model->groupStart();
                $model->orLike('pollstat.name', $search_text);
                $model->groupEnd();
            }
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("pollstat.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        {
            $model->where('pollstat.status', '1');
        }
        else
        {
            $model->where('pollstat.status', $filterArray['status']);
        }
        ...
        {
            $model->where('pollstat.deleted_at', NULL);
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("pollstat.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("pollstat.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $PollstatModel = new PollstatModel();
        if ($PollstatModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID PollstatModel Delete Error");
            return $this->fail($PollstatModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('pollstat._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'pollstat')
        {
            $data['breadcrumb'][] = ['url' => voters_url('pollstat/pollstat'), 'title' => lang('pollstat._page_pollstat')];
            $data['breadcrumb'][] = ['title' => lang('pollstat._form_pollstat'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $PollstatModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $PollstatModel->find($copy);
    ...
    //--------------------------------------------------------------------
    /**
     * Update single field of pollstat 
     * 
     * @param int $id
     * @return mix
     */
    public function update($id)
    {
        ...
        {
            if (!$this->validate([
                'status' => ['label' => lang('pollstat.status'), 'rules' => 'required|integer|max_length[1]|in_list[1,2]'],
            ]))
            {
   ...
   //--------------------------------------------------------------------
    ...
    public function readStatistic($pageSlug, $cardSlug)
    {
        ...
        //Page: pollstat
        if($pageSlug === 'pollstat' && $cardSlug === 'Polling-Stations')
        {
            $PollstatModel = new PollstatModel();
            $PollstatModel->select('COUNT(id) as COUNT_id');

            //---------------------------------------------------------------------// 
            $PollstatModel->where('pollstat.status', '1');
            //---------------------------------------------------------------------//
            $PollstatModel->where("deleted_at IS NULL");

            $cardData = $PollstatModel->get()->getRowArray();
        }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'c27959c']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'voters/Language/' . $locale . '/pollstat.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/pollstat.php';
        ...
        echo 'var LANG_pollstat = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'pollstat';

        if ($this->request->isAJAX())
        {
            try
            {
                echo voters_view('pollstat/' . $page, $data);
            }
            catch (\Exception $e)
            {
