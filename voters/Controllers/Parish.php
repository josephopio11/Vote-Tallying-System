...
class Parish extends BaseController
{
    ...
    {
        helper(['Voters\parish', 'Voters\subcounty']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '9ca9a2b']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'ee8656d']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'parish')
        {
            $data['breadcrumb'][] = ['url' => voters_url('parish/parish'), 'title' => lang('parish._page_parish')];
   ...
   //--------------------------------------------------------------------
    ...
    public function readStatistic($pageSlug, $cardSlug)
    {
        ...
        //Page: parish
        if($pageSlug === 'parish' && $cardSlug === 'Parishes')
        {
            $ParishModel = new ParishModel();
            $ParishModel->select('COUNT(id) as COUNT_id');

            //---------------------------------------------------------------------// 
            $ParishModel->where('parish.status', '1');
            //---------------------------------------------------------------------//
            $ParishModel->where("deleted_at IS NULL");

            $cardData = $ParishModel->get()->getRowArray();
        }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '6f067cf']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'voters/Language/' . $locale . '/parish.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/parish.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo voters_view('parish/' . $page, $data);
            }
            catch (\Exception $e)
            {
