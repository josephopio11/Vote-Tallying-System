...
use Voters\Models\UsertypeModel;
...
class Usertype extends BaseController
{
    ...
    {
        helper(['Voters\usertype']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = usertype 
     * usertype   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '8a8a9c8']);

        $data                 = ['page_title' => lang('usertype._page_usertype')];
        $data['breadcrumb'][] = ['url' => voters_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('usertype._page_usertype'), 'class' => 'active'];

        $this->_render('usertype', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = usertype
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'ff2b93d']);
     ...
     * @return json
     */
    public function save($form_slug, $type_id = '')
    {
        $data       = $this->request->getPost();
        $validation = \Config\Services::validation();

        //---------------------------------------------------------------------//
        //For form_slug=usertype pageSlug=usertype
        if($form_slug === 'usertype')
        {

            // usertype form Validations and security
            $validation->setRules([
                'usertype'      => ['label' => lang('usertype.usertype'), 'rules'=>"required|max_length[255]|string"],
                'comment'       => ['label' => lang('usertype.comment'), 'rules'=>"required|string"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['usertype', 'comment', 'type_id'];
        ...
        if(empty($type_id))
        {
            $type_id = $UsertypeModel->getInsertID();
        }

        return $this->respondCreated(['id' => $type_id, 'message' => lang('home.saved')]);
    }
     ...
     * 
     */
    public function readUsertype($pageSlug)
    {
        ...
        if($pageSlug === 'usertype')
        {
            $select_text       = "usertype.*";
            $multipleFields    = [];
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("usertype.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        {
            $model->where('usertype.status', '1');
        }
        else
        {
            $model->where('usertype.status', $filterArray['status']);
        }
        ...
        {
            $model->where('usertype.deleted_at', NULL);
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("usertype.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("usertype.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $UsertypeModel = new UsertypeModel();
        if ($UsertypeModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID UsertypeModel Delete Error");
            return $this->fail($UsertypeModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('usertype._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'usertype')
        {
            $data['breadcrumb'][] = ['url' => voters_url('usertype/usertype'), 'title' => lang('usertype._page_usertype')];
            $data['breadcrumb'][] = ['title' => lang('usertype._form_usertype'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $UsertypeModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $UsertypeModel->find($copy);
            
            if(isset($data['formData']['type_id']))
            {
                unset($data['formData']['type_id']);
            }
        }
    ...
    //--------------------------------------------------------------------
    /**
     * Update single field of usertype 
     * 
     * @param int $id
     * @return mix
     */
    public function update($id)
    {
        ...
        {
            if (!$this->validate([
                'status' => ['label' => lang('usertype.status'), 'rules' => 'required|integer|max_length[1]|in_list[1,2]'],
            ]))
            {
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '27d2b1d']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'voters/Language/' . $locale . '/usertype.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/usertype.php';
        ...
        echo 'var LANG_usertype = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'usertype';

        if ($this->request->isAJAX())
        {
            try
            {
                echo voters_view('usertype/' . $page, $data);
            }
            catch (\Exception $e)
            {
