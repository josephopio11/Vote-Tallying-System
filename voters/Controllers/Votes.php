...
class Votes extends BaseController
{
    ...
    {
        helper(['Voters\votes', 'Voters\c4_zone', 'Voters\county', 'Voters\subcounty', 'Voters\parish', 'Voters\pollstat', 'Voters\users']);
    }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '1686c21']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '5eadd67']);
    ...
    public function delete($id)
    {
        ...
        $VotesModel = new VotesModel();
        if ($VotesModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID VotesModel Delete Error");
            return $this->fail($VotesModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'votes')
        {
            $data['breadcrumb'][] = ['url' => voters_url('votes/votes'), 'title' => lang('votes._page_votes')];
    ...
    public function uploadFile($formSlug, $fieldName)
    {
        ...
        {
            if (!$this->validate(['file' => ['label' => lang('votes.evidence'), 'rules' => 'uploaded[file]|is_image[file]|max_size[file,2048]|max_dims[file,2048,2048]']]))
            {
                return $this->fail($this->validator->getErrors(), 400);
            }
            
            $return = $fileService->upload("votes/$fieldName", false);
        }
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '9a54675']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'voters/Language/' . $locale . '/votes.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/votes.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo voters_view('votes/' . $page, $data);
            }
            catch (\Exception $e)
            {
