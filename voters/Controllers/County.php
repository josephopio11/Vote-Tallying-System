...
use Voters\Models\C4_zoneModel;
...
class County extends BaseController
{
    ...
    {
        helper(['Voters\county', 'Voters\c4_zone']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = county 
     * county   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'a5309f3']);

        $data                 = ['page_title' => lang('county._page_county')];
        $data['breadcrumb'][] = ['url' => voters_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('county._page_county'), 'class' => 'active'];

        $this->_render('county', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = county
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '34d01fe']);
     ...
     * @return json
     */
    public function save($form_slug, $countyid = '')
    {
        $data       = $this->request->getPost();
        $validation = \Config\Services::validation();

        //---------------------------------------------------------------------//
        //For form_slug=county pageSlug=county
        if($form_slug === 'county')
        {

            // county form Validations and security
            $validation->setRules([
                'zoneid'        => ['label' => lang('county.zoneid'), 'rules'=>"required|integer|max_length[11]"],
                'name'          => ['label' => lang('county.name'), 'rules'=>"required|string|max_length[25]"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['zoneid', 'name', 'countyid'];
            $data             = array_intersect_key($data, array_flip($filterFormFields));
            //----------------------------------------------------------------//

            //----------------------------------------------------------------//
            /**
            * Check zoneid for security
            */        
            //----------------------------------------------------------------//
            if (empty($this->request->getPost('new_zoneid'))  && empty(getC4_zone($data['zoneid'])))
            {
                log_message('error', 'SECURITY: Undefined zoneid value posted');
                return $this->fail(['zoneid'=>'You Can not set this value'], 400, lang('home.error_validation_form')); 
            }
            //----------------------------------------------------------------//
        ...
        if(empty($countyid))
        {
            $countyid = $CountyModel->getInsertID();
        }

        return $this->respondCreated(['id' => $countyid, 'message' => lang('home.saved')]);
    }
     ...
     * 
     */
    public function readCounty($pageSlug)
    {
        ...
        if($pageSlug === 'county')
        {
            $select_text       = "county.*, c4_zone.name as c4_zone_RELATIONAL_name";
            $multipleFields    = [];

            //------------------------------------------------------------------//
            //merge relation fields names to make Sortable
            $table_colons = array_merge($table_colons, ['c4_zone_RELATIONAL_name']);
            //------------------------------------------------------------------//
            //------------------------------------------------------------------//        
            // Left Join Text
            //------------------------------------------------------------------//
            $model->join("(SELECT c4_zone_id, name FROM c4_zone GROUP BY c4_zone_id ORDER BY c4_zone_id DESC) c4_zone", 'c4_zone.c4_zone_id = county.zoneid', 'left');

            if (!empty($search_text))
            {
                $search_text = filter_var($search_text, FILTER_SANITIZE_STRING);
                $model->groupStart();
                $model->orLike('county.name', $search_text);
                $model->groupEnd();
            }
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("county.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        {
            $model->where('county.status', '1');
        }
        else
        {
            $model->where('county.status', $filterArray['status']);
        }
        ...
        {
            $model->where('county.deleted_at', NULL);
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("county.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("county.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $CountyModel = new CountyModel();
        if ($CountyModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID CountyModel Delete Error");
            return $this->fail($CountyModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('county._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'county')
        {
            $data['breadcrumb'][] = ['url' => voters_url('county/county'), 'title' => lang('county._page_county')];
            $data['breadcrumb'][] = ['title' => lang('county._form_county'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $CountyModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $CountyModel->find($copy);
            
            if(isset($data['formData']['countyid']))
            {
                unset($data['formData']['countyid']);
            }
        }
    ...
    public function update($id)
    {
        ...
        {
            if (!$this->validate([
                'status' => ['label' => lang('county.status'), 'rules' => 'required|integer|max_length[1]|in_list[1,2]'],
            ]))
            {
   ...
   //--------------------------------------------------------------------
    ...
    public function readStatistic($pageSlug, $cardSlug)
    {
        ...
        //Page: county
        if($pageSlug === 'county' && $cardSlug === 'Counties')
        {
            $CountyModel = new CountyModel();
            $CountyModel->select('COUNT(countyid) as COUNT_countyid');

            //---------------------------------------------------------------------// 
            $CountyModel->where('county.status', '1');
            //---------------------------------------------------------------------//
            $CountyModel->where("deleted_at IS NULL");

            $cardData = $CountyModel->get()->getRowArray();
        }
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '5fd4116']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'voters/Language/' . $locale . '/county.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'voters/Language/' . $conf->defaultLocale . '/county.php';
        ...
        echo 'var LANG_county = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'county';

        if ($this->request->isAJAX())
        {
            try
            {
                echo voters_view('county/' . $page, $data);
            }
            catch (\Exception $e)
            {
