...
class AuthenticationService extends BaseService
{
     ...
     * Secondly check users_remember token from cookie and database
     * 
     * @return bool
     * 
     */
    public function isLoggedIn(): bool 
    {
        ...
        $token = get_cookie('users_remember', true);
    ...
    public function set_login($memberData, $remember = false, $message = ''): bool
    {
        ...
        $this->session->set('users_firstname', $memberData['firstname'] ?? null);
        $this->session->set('users_lastname', $memberData['lastname'] ?? null);
        $this->session->set('users_email', $email ?? null);
        ...
        if ($remember)
        {
            ...
            set_cookie('users_remember', $token, $this->rememberTime);
    ...
    public function logout(): bool 
    {
        ...
        $this->session->remove('users_firstname');
        $this->session->remove('users_lastname');
        $this->session->remove('users_email');

        //delete Cookies
        helper('cookie');

        set_cookie('user_id', '', -3600);
        set_cookie("users_remember", '', -3600);
    ...
    {
        return $this->session->get('users_firstname');
    }
    ...
    {
        return $this->session->get('users_lastname');
    }
    ...
    {
        return $this->session->get('users_firstname') . ' ' . $this->session->get('users_lastname');
    }
    ...
    {
        return $this->session->get('users_email');
    }
    ...
    public function getAllData()
    {
        ...
        helper(['Voters\\c4_country', 'Voters\\c4_zone']);
